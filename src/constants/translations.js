export const translations = {
  th: {
    searchPlaceholder: "ค้นหาหมอ, แผนก...",
    idCardNumber: "หมายเลขบัตรประชาชน",
    welcomeBack: "ยินดีต้อนรับกลับ",
    howFeeling: "วันนี้คุณรู้สึกอย่างไร?",
    askAI: "ถาม AI",
    instantDoctor: "หมอทันที",
    bookAppointment: "นัดหมายล่วงหน้า",
    loadingDoctors: "กำลังโหลดรายชื่อแพทย์...",
    noDoctorsFound: "ไม่พบแพทย์",
    bookNow: "จองเลย",
    pageAppointments: "การนัดหมายของฉัน",
    pageAppointmentsDesc: "รายการนัดหมายที่กำลังจะมาถึงและที่ผ่านมาจะแสดงที่นี่",
    pageChat: "แชท",
    pageChatDesc: "ประวัติการแชทของคุณกับแพทย์และฝ่ายสนับสนุนจะอยู่ที่นี่",
    pageProfile: "โปรไฟล์ของฉัน",
    pageProfileDesc: "คุณสามารถแก้ไขข้อมูลส่วนตัวของคุณได้ที่นี่",
    accreditedBy: "รับรองโดย",
    case: "เคส",
    baht: "บาท",
    minute: "นาที",
    logout: "ออกจากระบบ",
    personalInformation: "ข้อมูลส่วนตัว",
    firstNameLastName: "ชื่อ-นามสกุล",
    specialty: "โรคเฉพาะทาง",
    gender: "เพศ",
    address: "ที่อยู่",
    province: "จังหวัด",
    district: "อำเภอ",
    subDistrict: "ตำบล",
    selectGender: "เลือกเพศ",
    male: "ชาย",
    female: "หญิง",
    other: "อื่นๆ",
    preferNotToSay: "ไม่ต้องการระบุ",
    placeholder: "วันเกิด",
    dayMonthYear: "วัน / เดือน / ปี",
    medicalHistory: "โรคประจำตัว",
    medicalHistoryPlaceholder: "ยังไม่มีประวัติโรค",
    queue: "จัดการคิว",
    pageQueueDesc: "รายการคิวและนัดหมายคนไข้ของคุณจะแสดงที่นี่",
    adminDashboard: "แดชบอร์ด",
    doctorManagement: "จัดการแพทย์",
    patientManagement: "จัดการผู้ป่วย",
    totalPatients: "ผู้ป่วยทั้งหมด",
    totalDoctors: "แพทย์ทั้งหมด",
    pageDoctorMgmtDesc: "รายชื่อแพทย์ทั้งหมดในระบบ",
    pagePatientMgmtDesc: "รายชื่อผู้ป่วยทั้งหมดในระบบ",
    email: "อีเมล",
    status: "สถานะ",
    action: "ดำเนินการ",
    active: "ใช้งาน",
    pending: "รออนุมัติ",
    approve: "อนุมัติ",
    loadingPatients: "กำลังโหลดรายชื่อผู้ป่วย...",
    noPatientsFound: "ไม่พบผู้ป่วย",
    saveProfile: "บันทึกโปรไฟล์",
    confirmLeave: "ยืนยันการแก้ไขหรือไม่? (ตกลง = บันทึก, ยกเลิก = ไม่บันทึก)",
    back: "ย้อนกลับ",
    selectDate: "เลือกวันที่",
    nextWeek: "สัปดาห์ถัดไป",
    selectTime: "เลือกเวลา",
    noDoctorSelected: "ยังไม่ได้เลือกแพทย์",
    noDoctorSelectedDesc: "โปรดกลับไปหน้าหลักเพื่อเลือกแพทย์ที่ต้องการนัดหมาย",
    goBack: "ย้อนกลับ",
    backToHome: "กลับสู่หน้าหลัก",
    selectDateAndTime: "เลือกวันและเวลา",
    calendarPlaceholder: "ปฏิทินจะแสดงที่นี่",
    confirmBooking: "ยืนยันการจอง",
    now: "ตอนนี้เลย",
    in30min: "ใน 30 นาที",
    in1hour: "ใน 1 ชั่วโมง",
    in2hours: "ใน 2 ชั่วโมง",
    addMedicalHistory: "เพิ่มประวัติการรักษา",
    diagnosis: "การวินิจฉัย",
    prescription: "ยาที่จ่าย",
    notes: "หมายเหตุ",
    cancel: "ยกเลิก",
    save: "บันทึก",
    viewMedicalHistory: "ดูประวัติการรักษา",
    patientName: "ชื่อคนไข้",
    loading: "กำลังโหลด...",
    noMedicalHistory: "ไม่มีประวัติการรักษา",
    close: "ปิด",
    add: "เพิ่ม",
    phone: "เบอร์โทรศัพท์",
    manageSchedule: "จัดการตารางเวลา",
    addUnavailability: "เพิ่มเวลาที่ไม่ว่าง",
    date: "วันที่",
    startTime: "เวลาเริ่มต้น",
    endTime: "เวลาสิ้นสุด",
    saveUnavailability: "บันทึกเวลาที่ไม่ว่าง",
    yourUnavailability: "เวลาที่ไม่ว่างของคุณ",
    time: "เวลา",
    noUnavailability: "ยังไม่มีการตั้งค่าเวลาที่ไม่ว่าง",
    fillAllFields: "กรุณากรอกข้อมูลให้ครบถ้วน",
    dbError: "ไม่สามารถเชื่อมต่อฐานข้อมูลได้ กรุณาลองใหม่อีกครั้ง",
    unavailabilitySaved: "บันทึกเวลาที่ไม่ว่างเรียบร้อยแล้ว!",
    unavailabilitySaveError: "ไม่สามารถบันทึกเวลาที่ไม่ว่างได้",
    confirmDelete: "คุณแน่ใจหรือไม่ว่าต้องการลบช่องเวลานี้",
    deleteSuccess: "ลบช่องเวลาเรียบร้อยแล้ว",
    deleteError: "ไม่สามารถลบช่องเวลาได้",
  },
  en: {
    searchPlaceholder: "search for doctor, specialties",
    idCardNumber: "ID card number",
    welcomeBack: "Welcome back",
    howFeeling: "How are you feeling today?",
    askAI: "Ask AI",
    instantDoctor: "Instant Doctor",
    bookAppointment: "Book Appointment",
    loadingDoctors: "Loading doctors...",
    noDoctorsFound: "No doctors found.",
    bookNow: "Book Now",
    pageAppointments: "My Appointments",
    pageAppointmentsDesc: "Your upcoming and past appointments will be listed here.",
    pageChat: "Chat",
    pageChatDesc: "Your chat history with doctors and support will be here.",
    pageProfile: "My Profile",
    pageProfileDesc: "You will be able to edit your personal information here.",
    accreditedBy: "Accredited by",
    case: "case",
    baht: "Baht",
    minute: "minute",
    logout: "Logout",
    personalInformation: "Personal Information",
    firstNameLastName: "First name Last name",
    specialty: "Specific Disease",
    gender: "Gender",
    address: "Address",
    province: "Province",
    district: "District",
    subDistrict: "Sub-district",
    selectGender: "Select Gender",
    male: "Male",
    female: "Female",
    other: "Other",
    preferNotToSay: "Prefer not to say",
    placeholder: "birthday",
    dayMonthYear: "Day / Month / Year",
    medicalHistory: "congenital disease",
    medicalHistoryPlaceholder: "No congenital disease added yet.",
    queue: "Queue",
    pageQueueDesc: "Your patient queue and appointments will be listed here.",
    adminDashboard: "Dashboard",
    doctorManagement: "Doctor Management",
    patientManagement: "Patient Management",
    totalPatients: "Total Patients",
    totalDoctors: "Total Doctors",
    pageDoctorMgmtDesc: "List of all doctors in the system.",
    pagePatientMgmtDesc: "List of all patients in the system.",
    email: "Email",
    status: "Status",
    action: "Action",
    active: "Active",
    pending: "Pending",
    approve: "Approve",
    loadingPatients: "Loading patients...",
    noPatientsFound: "No patients found.",
    saveProfile: "Save Profile",
    confirmLeave: "Confirm changes? (OK = Save, Cancel = Don't Save)",
    back: "Back",
    selectDate: "Select Date",
    nextWeek: "Next Week",
    selectTime: "Select Time",
    noDoctorSelected: "No Doctor Selected",
    noDoctorSelectedDesc: "Please go back to the home page to select a doctor for an appointment.",
    goBack: "Go Back",
    backToHome: "Back to Home",
    selectDateAndTime: "Select Date and Time",
    calendarPlaceholder: "Calendar will be displayed here.",
    confirmBooking: "Confirm Booking",
    now: "Now",
    in30min: "In 30 minutes",
    in1hour: "In 1 hour",
    in2hours: "In 2 hours",
    addMedicalHistory: "Add Medical History",
    diagnosis: "Diagnosis",
    prescription: "Prescription",
    notes: "Notes",
    cancel: "Cancel",
    save: "Save",
    viewMedicalHistory: "View Medical History",
    patientName: "Patient Name",
    loading: "Loading...",
    noMedicalHistory: "No medical history available",
    close: "Close",
    add: "Add",
    phone: "Phone Number",
    manageSchedule: "Manage Schedule",
    addUnavailability: "Add Unavailable Time",
    date: "Date",
    startTime: "Start Time",
    endTime: "End Time",
    saveUnavailability: "Save Unavailable Time",
    yourUnavailability: "Your Unavailable Times",
    time: "Time",
    noUnavailability: "No unavailable times set.",
    fillAllFields: "Please fill all fields",
    dbError: "Database not available. Please try again later.",
    unavailabilitySaved: "Unavailable time saved successfully!",
    unavailabilitySaveError: "Failed to save unavailable time.",
    confirmDelete: "Are you sure you want to delete this unavailable slot?",
    deleteSuccess: "Slot deleted successfully.",
    deleteError: "Failed to delete slot.",
  }
};
